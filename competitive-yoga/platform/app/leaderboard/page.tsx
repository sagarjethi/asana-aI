 ...
 * Accepts an optional ?roundId; fetches /api/leaderboard?roundId and renders
 * ranked rows (rank, name, country, total, published badge). Auto-refreshes.
 */
...
function rankRing(rank: number): string {
  if (rank === 1) return "ring-2 ring-sun-400";
  if (rank <= 3) return "ring-1 ring-sun-300";
  return "";
}
...
function LeaderboardInner() {
  ...
  React.useEffect(() => {
    let cancelled = false;
    const url = roundId
      ? `/api/leaderboard?roundId=${encodeURIComponent(roundId)}`
      : "/api/leaderboard";
    const load = () =>
      fetch(url)
    ...
    <div className="mx-auto max-w-3xl px-4 py-10 sm:px-6">
      <header className="flex items-end justify-between gap-3">
        <div>
          ...
          {roundId ? (
            <p className="mt-1 text-xs text-sun-900/60">Round {roundId}</p>
          ) : null}
        </div>
              ...
              >
                ...
                <span className="min-w-0">
                  <span className="flex items-center gap-2">
                    ...
                    {row.published ? <Badge tone="success">published</Badge> : null}
                  </span>
                  {row.country ? (
